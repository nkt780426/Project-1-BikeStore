...
public class AccountsDao {
	...
	public boolean checkPassword(Accounts account) {
		Connection connection = JDBCUtil.getConnection();
		try {
			String sql = "SELECT username FROM sales.accounts WHERE username = ?, password COLLATE Latin1_General_CS_AS = ?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, account.getUsername());
			preparedStatement.setString(2, account.getPassword());
			ResultSet rs = preparedStatement.executeQuery();
	...
	public boolean checkAccount(Accounts account) {
		Connection connection = JDBCUtil.getConnection();
		try {
			String sql = "SELECT staffId FROM sales.accounts WHERE username COLLATE Latin1_General_CS_AS like ? and password COLLATE Latin1_General_CS_AS like ? and position like ? ";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, account.getUsername());
			preparedStatement.setString(2, account.getPassword());
			preparedStatement.setBoolean(3, account.getPosition());
			...
			while (rs.next()) {
				if (rs.getObject("staffId") != null) {
					account.setStaffId(rs.getInt("staffId"));
				} else {
